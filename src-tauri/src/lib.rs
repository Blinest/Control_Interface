...
fn diagnostics_summary_from_runtime(
    ...
    DiagnosticsSummary {
        ...
        protocol_errors: runtimes
            ...
            .sum(),
        valid_frames: runtimes
            .iter()
            .map(|(_, status)| status.protocol_stats.valid_frames)
            .sum(),
        invalid_frames: runtimes
            .iter()
            .map(|(_, status)| status.protocol_stats.invalid_frames)
            .sum(),
        checksum_errors: runtimes
            .iter()
            .map(|(_, status)| status.protocol_stats.checksum_errors)
            .sum(),
        decode_errors: runtimes
            .iter()
            .map(|(_, status)| status.protocol_stats.decode_errors)
            .sum(),
        ...
        last_protocol_error: runtimes
            .iter()
            .rev()
            .find_map(|(_, status)| status.protocol_stats.last_error.clone()),
    }
}
...
fn apply_diagnostics_summary(
    ...
    let summary = diagnostics_summary_from_runtime(live_stats, runtimes);
...
fn build_snapshot(data: &PersistedState) -> RuntimeSnapshot {
    ...
    RuntimeSnapshot {
        ...
        runtime_diagnostics: diagnostics_summary_from_runtime(&empty_live_stats(), &[]),
...
struct AppState {
    ...
    synced_log_ids: Mutex<HashSet<u64>>,
    worker_stop: Arc<AtomicBool>,
}

fn pending_logs_for_sqlite(logs: &[LogEntry], synced: &HashSet<u64>) -> Vec<LogEntry> {
    logs.iter()
        .filter(|entry| !synced.contains(&entry.id))
        .cloned()
        .collect()
}
...
impl AppState {
    fn new(path: PathBuf) -> Self {
        ...
        Self {
            ...
            synced_log_ids: Mutex::new(HashSet::new()),
    ...
    fn sync_logs_to_sqlite(&self) {
        let logs = self
            ...
            .unwrap_or_default();
        let Ok(mut synced) = self.synced_log_ids.lock() else {
            return;
        };
        let pending = pending_logs_for_sqlite(&logs, &synced);
        for entry in pending {
            if self
                .sqlite
                .insert_log(&storage::LogRow {
                    id: entry.id,
...
fn command_rejected_view(device_id: &str, message: &str) -> CommandResultView {
    CommandResultView {
...
fn runtime_allows_command(status: &device::RuntimeStatus, safety: CommandSafety) -> bool {
    if status.emergency_latched || status.state == device::DeviceConnectionState::EmergencyStopped {
        return safety == CommandSafety::Recovery
            && status.state == device::DeviceConnectionState::EmergencyStopped;
    }
    ...
    match safety {
        CommandSafety::Connected | CommandSafety::Recovery => connected,
...
fn guard_command_allowed(
    ...
    guard_command_allowed_in_app_state(state, device_id, safety)
}

fn guard_command_allowed_in_app_state(
    state: &AppState,
    device_id: &str,
    safety: CommandSafety,
) -> Result<(), String> {
    let required_permission = match safety {
        CommandSafety::Connected | CommandSafety::Recovery => auth::Permission::ConnectDevice,
        CommandSafety::Enabled => auth::Permission::SendMotionCommand,
    };
    guard_permission_or_reject_in_app_state(state, device_id, required_permission)?;
    ...
    let message = match safety {
        CommandSafety::Connected | CommandSafety::Recovery => "device is not connected or ready",
        CommandSafety::Enabled => {
            "device control is not enabled; enable control and wait for device feedback before sending motion commands"
        }
    };
    reject_command(state, device_id, message)
}

fn guard_permission_or_reject_in_app_state(
    state: &AppState,
    device_id: &str,
    ...
    match permission_result {
        Ok(()) => Ok(()),
        Err(message) => reject_command(state, device_id, &message),
    }
}

fn guard_system_control_allowed_in_app_state(
    state: &AppState,
    device_id: &str,
    action: &SystemControlActionRequest,
) -> Result<(), String> {
    match system_control_safety(action) {
        Some(safety) => guard_command_allowed_in_app_state(state, device_id, safety),
        None => Ok(()),
    }
}

fn system_control_safety(action: &SystemControlActionRequest) -> Option<CommandSafety> {
    match action {
        SystemControlActionRequest::Disable => Some(CommandSafety::Connected),
        SystemControlActionRequest::Enable => Some(CommandSafety::Recovery),
        // Emergency stop must remain available even when auth or connection state is unhealthy.
        SystemControlActionRequest::EmergencyStop => None,
    }
}

fn reject_command(state: &AppState, device_id: &str, message: &str) -> Result<(), String> {
    let result = command_rejected_view(device_id, message);
...
fn command_audit_payload<T: Serialize>(
    device_id: &str,
    command: &str,
    payload: &T,
    runtime: Option<&device::RuntimeStatus>,
) -> serde_json::Value {
    let result = runtime
        .map(|runtime| command_accepted_view(device_id, command, runtime))
        .unwrap_or_else(|| CommandResultView {
            status: "accepted".to_string(),
            device_id: device_id.to_string(),
...
fn record_control_command<T: Serialize>(
    ...
    let runtime = if device_id.starts_with("serial:") {
        state
            .devices
            .lock()
            .ok()
            .and_then(|devices| devices.runtime_status(device_id).ok())
    } else {
        None
    };
    let payload = command_audit_payload(device_id, command, payload, runtime.as_ref());
...
fn live_window(
    ring: &live::LiveDataRing,
    count: usize,
    device_id: Option<&str>,
) -> Vec<DeviceSnapshot> {
    match device_id {
        Some(device_id) => ring.window_for_device(device_id, count),
        None => ring.window(count),
    }
}
...
fn fetch_live_window(
    ...
    state
        .live_ring
        .lock()
        .map(|ring| live_window(&ring, count as usize, device_id.as_deref()))
        .unwrap_or_default()
}
...
fn submit_system_control(
    ...
    guard_system_control_allowed_in_app_state(&state, &device_id, &request.action)?;
...
fn calibrate_sensor(
    ...
    let device_id = selected_device_id(request.device_id);
    guard_permission_or_reject_in_app_state(&state, &device_id, auth::Permission::RunCalibration)?;
...
mod tests {
    ...
    #[test]
    fn command_result_view_marks_guard_rejections() {
        let result = command_rejected_view(
            "serial:COM3",
            "device control is not enabled; enable control and wait for device feedback before sending motion commands",
        );

        assert_eq!(result.status, "rejected");
        assert_eq!(result.device_id, "serial:COM3");
        assert!(result.message.contains("device control is not enabled"));
        assert_eq!(result.pending_commands, 0);
        assert_eq!(result.sent_commands, 0);
        assert!(!result.emergency_latched);
    }

    #[test]
    fn command_result_view_reflects_runtime_status() {
        let runtime = device::RuntimeStatus {
            ...
            last_frame_ms: 0,
            last_command_ms: 10,
            command_high_watermark: 3,
            emergency_latched: true,
            last_error: None,
            last_error_code: None,
            protocol_stats: protocol::ProtocolStats::default(),
        };

        let result = command_accepted_view("serial:COM3", "emergency stop", &runtime);

        assert_eq!(result.status, "accepted");
        assert_eq!(result.message, "emergency stop");
        assert_eq!(result.sent_commands, 4);
        assert!(result.emergency_latched);
    }

    #[test]
    fn command_result_view_logs_permission_guard_rejections() {
        let test_dir = PathBuf::from("D:\\APP\\ControlUI\\data").join(format!(
            "task-5-command-result-permission-{}-{}",
            std::process::id(),
            now_ms()
        ));
        let state = AppState::new(test_dir.join("softui-state.json"));

        let error =
            guard_command_allowed_in_app_state(&state, "softui-sim-01", CommandSafety::Connected)
                .expect_err("signed-out command should be rejected");
        let logs = state
            .store
            .data
            .lock()
            ...
            .logs
            .clone();
        let log = logs.last().expect("permission rejection log");

        assert!(error.starts_with("permission denied:"));
        assert_eq!(log_level_key(&log.level), "warn");
        assert_eq!(log.scope, "control");
        assert_eq!(log.device_id.as_deref(), Some("softui-sim-01"));
        assert_eq!(log.message, format!("rejected: {error}"));

        drop(state);
        fs::remove_dir_all(&test_dir).expect("remove test runtime data");
    }

    #[test]
    fn signed_out_enable_is_rejected_through_structured_audit() {
        let test_dir = PathBuf::from("D:\\APP\\ControlUI\\data").join(format!(
            "final-fix-enable-permission-{}-{}",
            std::process::id(),
            now_ms()
        ));
        let state = AppState::new(test_dir.join("softui-state.json"));

        let error = guard_system_control_allowed_in_app_state(
            &state,
            "softui-sim-01",
            &SystemControlActionRequest::Enable,
        )
        .expect_err("signed-out enable should be rejected");
        let logs = state
            .store
            .data
            .lock()
            ...
            .logs
            .clone();
        let log = logs.last().expect("enable rejection log");
    ...
    #[test]
    fn sqlite_log_sync_filters_already_synced_ids() {
        let logs = vec![
            log_entry(1, LogLevel::Info, "boot", "ready", None, None),
            log_entry(
                2,
                LogLevel::Warn,
                "device",
        ...
        let mut synced = std::collections::HashSet::from([1]);

        let pending = pending_logs_for_sqlite(&logs, &synced);
        for entry in &pending {
            synced.insert(entry.id);
        }
        let repeated = pending_logs_for_sqlite(&logs, &synced);
    ...
    #[test]
    fn signed_out_emergency_stop_bypasses_normal_command_guards() {
        let test_dir = PathBuf::from("D:\\APP\\ControlUI\\data").join(format!(
            "final-fix-emergency-permission-{}-{}",
            std::process::id(),
            now_ms()
        ));
        let state = AppState::new(test_dir.join("softui-state.json"));

        let result = guard_system_control_allowed_in_app_state(
            &state,
            "softui-sim-01",
            &SystemControlActionRequest::EmergencyStop,
        );
    ...
    #[test]
    fn signed_out_calibration_is_rejected_through_structured_audit() {
        let test_dir = PathBuf::from("D:\\APP\\ControlUI\\data").join(format!(
            "final-fix-calibration-permission-{}-{}",
            std::process::id(),
            now_ms()
        ));
        let state = AppState::new(test_dir.join("softui-state.json"));

        let error = guard_permission_or_reject_in_app_state(
            &state,
            "softui-sim-01",
            auth::Permission::RunCalibration,
        )
        .expect_err("signed-out calibration should be rejected");
        let logs = state
            .store
            .data
            .lock()
            ...
            .logs
            .clone();
        let log = logs.last().expect("calibration rejection log");
    ...
    #[test]
    fn live_window_filters_device_and_preserves_chronological_order() {
        let mut ring = live::LiveDataRing::new(10);
        let mut first = make_frame(1, true);
        first.device_id = "dev-a".to_string();
        let mut other = make_frame(2, true);
        other.device_id = "dev-b".to_string();
        let mut newest = make_frame(3, true);
        newest.device_id = "dev-a".to_string();
        ring.push(first);
        ring.push(other);
        ring.push(newest);

        let frames = live_window(&ring, 10, Some("dev-a"));
        ...
        assert!(frames.iter().all(|frame| frame.device_id == "dev-a"));
    }

    #[test]
    fn command_result_view_audits_simulator_acceptance() {
        let payload = command_audit_payload(
            "softui-sim-01",
            "motor command sent",
            &serde_json::json!({ "motorId": 1 }),
            None,
        );

        assert_eq!(payload["command"]["motorId"], 1);
        assert_eq!(payload["result"]["status"], "accepted");
        assert_eq!(payload["result"]["deviceId"], "softui-sim-01");
        assert_eq!(payload["result"]["message"], "motor command sent");
        assert_eq!(payload["result"]["pendingCommands"], 0);
        assert_eq!(payload["result"]["sentCommands"], 0);
        assert_eq!(payload["result"]["emergencyLatched"], false);
    }
    ...
    #[test]
    fn authorized_serial_enable_allows_emergency_recovery_while_motion_stays_blocked() {
        let runtime = device::RuntimeStatus {
            state: device::DeviceConnectionState::EmergencyStopped,
            last_status: None,
            received_frames: 1,
            ...
            last_frame_ms: 0,
            last_command_ms: 0,
            command_high_watermark: 1,
            emergency_latched: true,
            last_error: None,
            last_error_code: None,
        };
        let session = auth::AuthSession::for_role("operator", auth::Role::Operator);
        let enable_safety = system_control_safety(&SystemControlActionRequest::Enable)
            .expect("enable safety policy");

        assert!(session.has_permission(auth::Permission::ConnectDevice));
        assert_eq!(enable_safety, CommandSafety::Recovery);
        assert!(runtime_allows_command(&runtime, enable_safety));
        assert!(!runtime_allows_command(&runtime, CommandSafety::Enabled));
    }
    ...
    fn diagnostics_summary_aggregates_live_and_runtime_state() {
        ...
        let summary =
            diagnostics_summary_from_runtime(&stats, &[("serial:COM7".to_string(), runtime)]);
    ...
    #[test]
    fn diagnostics_summary_includes_protocol_stats() {
        let live_stats = live::FrameStats {
            stored_frames: 3,
            capacity: 10,
            total_frames: 3,
            dropped_frames: 0,
        ...
        let runtime = device::RuntimeStatus {
            ...
            last_error: None,
            last_error_code: None,
            protocol_stats: protocol::ProtocolStats {
                valid_frames: 2,
                invalid_frames: 1,
                checksum_errors: 1,
                decode_errors: 0,
                last_error: Some("InvalidChecksum".to_string()),
                last_frame_ms: 10,
            },
        };

        let summary =
            diagnostics_summary_from_runtime(&live_stats, &[("serial:COM3".to_string(), runtime)]);
        ...
        assert_eq!(
            summary.last_protocol_error.as_deref(),
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            let state_path = softui_data_dir(app).join("softui-state.json");
...
fn softui_data_dir(app: &tauri::App) -> PathBuf {
    if let Ok(path) = std::env::var("SOFTUI_DATA_DIR") {
        if !path.trim().is_empty() {
            return PathBuf::from(path);
        }
    }

    let preferred = PathBuf::from(r"D:\APP\ControlUI\data");
