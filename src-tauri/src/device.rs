...
impl<T: Transport> DeviceRuntime<T> {
    ...
    pub fn poll_status(&mut self) -> Result<DeviceStatus, DeviceRuntimeError> {
        ...
        let was_handshaking = self.state == DeviceConnectionState::Handshaking;
    ...
    pub fn enqueue_command_result(
        ...
        self.enqueue_command(frame, priority);
        let status = self.status();
        CommandQueueResult {
            status: CommandQueueStatus::Accepted,
            pending_commands: status.pending_commands,
            sent_commands: status.sent_commands,
            message: None,
        }
    }
...
mod tests {
    ...
    use crate::{
        protocol::{
            encode_motor_command, encode_status_frame, encode_system_control_command, DeviceStatus,
            SystemControlAction,
        },
    ...
    fn status_frame(bend_angle1_deg: f64) -> (DeviceStatus, Vec<u8>) {
    ...
    #[test]
    fn poll_status_applies_all_valid_frames_and_returns_newest() {
        let (_, first) = status_frame(1.0);
        let (newest, second) = status_frame(2.0);
        let mut batch = first;
        batch.extend(second);
        let transport = FragmentedTransport {
            chunks: vec![batch],
            writes: Vec::new(),
        };
        let mut runtime = DeviceRuntime::new(transport);

        let status = runtime.poll_status().expect("newest status");

        assert_eq!(status, newest);
        assert_eq!(runtime.status().received_frames, 2);
        assert_eq!(runtime.status().protocol_errors, 0);
    }

    #[test]
    fn poll_status_records_bad_checksum_before_later_valid_frame() {
        let (_, mut bad) = status_frame(1.0);
        let (valid, valid_frame) = status_frame(2.0);
        let checksum_index = bad.len() - 1;
        bad[checksum_index] ^= 0xFF;
        bad.extend(valid_frame);
        let transport = FragmentedTransport {
            chunks: vec![bad],
            writes: Vec::new(),
        };
        ...
        let status = runtime
            .poll_status()
            .expect("valid status after checksum error");
        let runtime_status = runtime.status();

        assert_eq!(status, valid);
        assert_eq!(runtime_status.received_frames, 1);
        assert_eq!(runtime_status.protocol_errors, 1);
        assert_eq!(runtime_status.protocol_stats.checksum_errors, 1);
    }
    ...
    #[test]
    fn runtime_status_exposes_codec_protocol_stats() {
        let transport = SimulatorTransport::new().expect("simulator");
        let mut runtime = DeviceRuntime::new(transport);

        let _ = runtime.poll_status();

        let status = runtime.status();
        assert_eq!(status.protocol_stats.valid_frames, status.received_frames);
        assert_eq!(status.protocol_stats.checksum_errors, 0);
    }

    #[test]
    fn enqueue_command_result_reports_accepted_normal_command() {
        let transport = SimulatorTransport::new().expect("simulator");
        let mut runtime = DeviceRuntime::new(transport);
        let command = encode_motor_command(2, 10.0, 12.0, 4.0).expect("command");

        let result = runtime.enqueue_command_result(command, CommandPriority::Normal);

        assert_eq!(result.status, CommandQueueStatus::Accepted);
        assert_eq!(result.pending_commands, 1);
        assert_eq!(result.sent_commands, 0);
    }

    #[test]
    fn enqueue_command_result_reports_emergency_as_executed_after_flush() {
        let transport = SimulatorTransport::new().expect("simulator");
        let mut runtime = DeviceRuntime::new(transport);
        let normal = encode_motor_command(2, 10.0, 12.0, 4.0).expect("normal");
        let emergency =
            encode_system_control_command(SystemControlAction::EmergencyStop).expect("emergency");
        let _ = runtime.enqueue_command_result(normal, CommandPriority::Normal);

        let result = runtime.enqueue_command_result(emergency, CommandPriority::Emergency);
        runtime.flush_command_queue().expect("flush");

        assert_eq!(result.status, CommandQueueStatus::Accepted);
        assert_eq!(runtime.status().pending_commands, 0);
        assert_eq!(runtime.status().sent_commands, 1);
