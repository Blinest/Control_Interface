        ...
        while index + 3 <= self.buffer.len() {
            ...
            match parse_status_frame(&frame) {
                Ok(status) => {
                    ...
                    self.stats.last_frame_ms = SystemTime::now()
                ...
                Err(ProtocolError::InvalidChecksum) => {
                    ...
                    self.stats.last_error = Some("InvalidChecksum".to_string());
                ...
                Err(error) => {
                    ...
                    self.stats.last_error = Some(format!("{error:?}"));
...
mod tests {
    ...
    fn sample_status() -> DeviceStatus {
        parse_status_frame(&sample_status_frame()).expect("status frame")
    }
    ...
    #[test]
    fn codec_stats_count_valid_status_frames() {
        let status = sample_status();
        let frame = encode_status_frame(&status).expect("status frame");
        let mut codec = LegacyV1Codec::new();

        let decoded = codec.push_bytes(&frame);
        ...
        assert!(stats.last_frame_ms > 0);
        assert_eq!(stats.last_error, None);
    }

    #[test]
    fn codec_stats_count_checksum_failures() {
        let status = sample_status();
        let mut frame = encode_status_frame(&status).expect("status frame");
        ...
        assert_eq!(decoded, vec![Err(ProtocolError::InvalidChecksum)]);
        let stats = codec.stats();
        assert_eq!(stats.valid_frames, 0);
        assert_eq!(stats.invalid_frames, 1);
        assert_eq!(stats.checksum_errors, 1);
        assert_eq!(stats.decode_errors, 0);
        assert_eq!(stats.last_error.as_deref(), Some("InvalidChecksum"));
    }

    #[test]
    fn codec_stats_reset_clears_counters() {
        let status = sample_status();
        let frame = encode_status_frame(&status).expect("status frame");
        let mut codec = LegacyV1Codec::new();
        let _ = codec.push_bytes(&frame);
