...
impl LiveDataRing {
    ...
    pub(crate) fn latest_for_device(&self, device_id: &str) -> Option<DeviceSnapshot> {
        self.buffer
            .iter()
            .rev()
            .find(|frame| frame.device_id == device_id)
            .cloned()
    }

    pub(crate) fn window_for_device(&self, device_id: &str, count: usize) -> Vec<DeviceSnapshot> {
        let mut frames = self
            .buffer
            .iter()
            .rev()
            .filter(|frame| frame.device_id == device_id)
            .take(count)
            .cloned()
            .collect::<Vec<_>>();
        frames.reverse();
...
mod tests {
    ...
    #[test]
    fn latest_for_device_returns_newest_matching_frame() {
        let mut ring = LiveDataRing::new(10);
        ring.push(dummy_frame("dev-a", 1, 1_000));
        ring.push(dummy_frame("dev-b", 2, 1_010));
        ring.push(dummy_frame("dev-a", 3, 1_020));

        let latest = ring.latest_for_device("dev-a").expect("latest frame");

        assert_eq!(latest.device_id, "dev-a");
        assert_eq!(latest.sequence, 3);
    }

    #[test]
    fn window_for_device_returns_chronological_bounded_frames() {
        let mut ring = LiveDataRing::new(10);
        ring.push(dummy_frame("dev-a", 1, 1_000));
        ring.push(dummy_frame("dev-b", 2, 1_010));
        ring.push(dummy_frame("dev-a", 3, 1_020));
        ring.push(dummy_frame("dev-a", 4, 1_030));

        let window = ring.window_for_device("dev-a", 2);
